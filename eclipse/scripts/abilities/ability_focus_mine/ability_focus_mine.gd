...
func activate(context: Dictionary) -> void:
	...
	if pressed:
		if not exploded:
			if not charging:
				# Auto-target: find the nearest mineable cluster from the player.
...
## Auto-select the nearest cluster of mineable tiles within cast_range.
## Flood-fills outward from the nearest tile to the player.
func _get_focus_tiles(player: Node, tilemap: Node) -> Array[Vector2i]:
	...
	# Find the nearest mineable tile to the player within cast_range.
	var start: Vector2i
	var result: Util.TargetingResult = Targeting.nearest_tile(player, tilemap, cast_range)
	if result.found:
		start = tilemap.world_to_map(result.position)
	else:
		# Nothing in range — fall back to the tile directly in front of the player.
		start = tilemap.world_to_map(player.global_position) + player.direction

	if tilemap.is_air(start):
		return []

	# BFS flood-fill up to `limit` connected tiles.
	var visited: Dictionary      = { start: true }
	var queue:   Array[Vector2i] = [start]
	var tiles:   Array[Vector2i] = []
	while queue.size() > 0 and tiles.size() < limit:
