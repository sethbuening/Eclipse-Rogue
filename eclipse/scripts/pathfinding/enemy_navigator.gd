# enemy_navigator.gd
# Child node of any Enemy. Call navigate_toward(target, delta) each physics frame.
extends Node

@export var move_speed:        float = 60.0
@export var path_update_rate:  float = 0.15
@export var arrival_threshold: float = 4.0
@export var stuck_timeout:     float = 0.4    # seconds before forcing a repath
...
func navigate_toward(target: Vector2, delta: float) -> Vector2:
	...
	# stuck detection — force repath if barely moving
	_stuck_timer += delta
	if _stuck_timer >= stuck_timeout:
		_stuck_timer = 0.0
		if _parent.global_position.distance_to(_last_position) < 2.0:
			_path = PackedVector2Array()   # force repath next frame
